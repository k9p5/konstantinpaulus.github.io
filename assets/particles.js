...
class Particle {
	constructor(x, y) {
		...
		this.baseX = this.x;
		this.baseY = this.y;
		this.density = Math.random() * cfg.density.max - cfg.density.min + cfg.density.min;
	}
...
(function () {
	...
	for (let y = 0, y2 = dataCoordinates.height; y < y2; y++) {
		for (let x = 0, x2 = dataCoordinates.width; x < x2; x++) {
			if (dataCoordinates.data[y * 4 * dataCoordinates.width + x * 4 + 3] > cfg.activeThreshold) {
				let positionX = (x + cfg.adjustX) * cfg.spread;
				let positionY = (y + cfg.adjustY) * cfg.spread;
